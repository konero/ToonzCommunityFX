...
vec3 layerBlendingAlgorithm(vec3 base, vec3 top) {
    // Pin light blending
    vec3 darkBlend = 2.0 * top;
    vec3 lightBlend = 2.0 * top - 1.0;
    
    return vec3(
        (top.r < 0.5) ? min(base.r, darkBlend.r) : max(base.r, lightBlend.r),
        (top.g < 0.5) ? min(base.g, darkBlend.g) : max(base.g, lightBlend.g),
        (top.b < 0.5) ? min(base.b, darkBlend.b) : max(base.b, lightBlend.b)
    );
}
